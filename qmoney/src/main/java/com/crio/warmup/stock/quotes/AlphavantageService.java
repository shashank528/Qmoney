...
public class AlphavantageService implements StockQuotesService {
  ...
  JsonProcessingException, StockQuoteServiceException, RuntimeException{
    // TODO Auto-generated method stub
 

    List<Candle> ans = new ArrayList<Candle>();
    Map<LocalDate, AlphavantageCandle> response;
...
//       response[i].getCandles().forEach((k,v) -> {
//         if( (k.isAfter(from) && k.isBefore(to)) || k.isEqual(to) || k.isEqual(from) )
//           {
//           AlphavantageCandle alphaCandle = v;
//           alphaCandle.setDate(k);
//           ans.add(alphaCandle);
...
try{
  ...
  response = obj.readValue(stocks,AlphavantageDailyResponse.class).getCandles();


  if(response!=null){
      response.forEach((k,v) -> {
        if( (!k.isAfter(to) && !k.isBefore(from)) )
          {
          AlphavantageCandle alphaCandle = v;
          alphaCandle.setDate(k);
          ans.add(alphaCandle);
  ...
  Comparator<Candle> res2 = Comparator.comparing(Candle::getDate);

  Collections.sort(ans, res2);
 ...
 return ans;
  ...
  protected String buildUri(String symbol) {
    String apikey = "N5LDTPFAS7NOGYX7";
  ...
  //       ans contains the records for for startDate and endDate after filtering.
